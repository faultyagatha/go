...
// solution: adapter that implements
// RasterImage interface
//  ---------------
...
func (a *vectorToRasterAdapter) addLineCached(line Line) {
	...
	if pts, ok := pointCache[h]; ok {
		for _, pt := range pts {
			//dont' calculate the points
			//again if they are already in the cache
